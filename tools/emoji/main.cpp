...
static int emit(FILE *fp, const char *bin, const FlexBuffer<uint8_t> &buf,
  ...
  fputs("\n};\n\n", fp);
  fprintf(fp, "constexpr uint16_t max_packed_emoji_code_point_count = %d;\n\n", maxCodePointCount);
...
int main(int argc, char **argv) {
  ...
  for (auto [p, codes] : emoji_seq_table) {
    ...
    maxCodeCount = std::max(maxCodeCount, codes.usedSize);
  ...
  if (maxCodeCount != tree.maxCodePointCount()) {
    fatal("longestStrigSize mismatched, expected: %d, actual: %d\n", maxCodeCount,
          tree.maxCodePointCount());
  }
  if (maxBytes >= static_cast<size_t>(UINT16_MAX) || maxCodeCount >= UINT16_MAX) {
    fatal("contains too large string\n");
  }

  // check trie
  const PackedRadixTree emojiTrie(maxCodeCount, buf.data(), buf.size());
  for (auto [p, codes] : emoji_seq_table) {
    auto str = codes.toUTF8();
  ...
  return emit(fp, argv[0], buf, static_cast<uint16_t>(maxCodeCount));
}