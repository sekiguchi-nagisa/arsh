...
TEST_F(KeyCodeTest, alt4) {
  static const struct {
    ...
    Optional<KeyEvent> event;
      ...
      {ESC_("\x09"), KeyEvent(FunctionKey::TAB, ModifierKey::ALT)},
      ...
      {ESC_("\x0D"), KeyEvent(FunctionKey::ENTER, ModifierKey::ALT)},
      ...
      {ESC_("\x1B"), KeyEvent(FunctionKey::ESCAPE, ModifierKey::ALT)},
      ...
      {ESC_("\x7F"), KeyEvent(FunctionKey::BACKSPACE, ModifierKey::ALT)},
      {ESC_("\x80"), {}},
  };
  for (unsigned int i = 0; i < std::size(patterns); i++) {
    auto &p = patterns[i];
