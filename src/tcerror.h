...
DEFINE_TCError(InsideChild, "some no-return expressions (break, continue, return) "
                            "are not allowed inside sub-shell and pipeline");
...
DEFINE_TCError(GlobResource, "not enough resources for glob expansion%s");
...
DEFINE_TCError(NeedFd, "`>&', `<&' redirection only allow decimal numbers (0~9), "
                       "`-' or `FD' type expression");
