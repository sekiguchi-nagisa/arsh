...
static constexpr struct {
  ...
  const char *detail;
    ...
    {":", builtin_true, "", "    Null command.  Always success (exit status is 0)."},
    {"__gets", builtin___gets, "", "    Read standard input and write to standard output."},
    ...
    {"eval", nullptr, "[arg ...]", "    Evaluate ARGs as command."},
    ...
    {"exit", builtin_exit, "[n]",
     "    Exit the shell with a status of N.  If N is omitted, the exit\n"
     "    status is $?."},
    {"false", builtin_false, "", "    Always failure (exit status is 1)."},
    ...
    {"setenv", builtin_setenv, "[name=env ...]", "    Set environmental variables."},
    ...
    {"true", builtin_true, "", "    Always success (exit status is 0)."},
